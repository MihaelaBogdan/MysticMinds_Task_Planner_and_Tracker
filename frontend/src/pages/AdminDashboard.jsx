...
function AdminDashboard() {
    ...
    const handleDeleteClick = async (user) => {
        if (user.role === 'manager') {
            ...
            if (!confirm('Stergi acest executor?')) return;
            try {
    ...
    const handleDeleteManager = async () => {
        ...
        try {
            ...
            if (managerExecutors.length > 0) {
                if (deleteAction === 'reassign') {
                    if (!reassignToManagerId) {
                        setError('Selecteaza un manager pentru reasignare!');
                        return;
                    }
            ...
            await fetch(`${API_URL}/users/${managerToDelete.id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
        ...
        <div className="app-container">
            ...
            <main className="main-content">
                            ...
                            <div key={user.id} className="user-card fade-in" style={{ cursor: 'pointer' }} onClick={() => viewUserTasks(user)}>
                                ...
                                <div className="user-card-info">
                                    ...
                                    <div className="user-card-email" style={{ color: '#888', fontSize: '0.75rem' }}>
                                        {users.filter(u => u.managerId === user.id).length} executori
                                    </div>
                                </div>
                ...
                <Modal isOpen={showDeleteManagerModal} onClose={() => { setShowDeleteManagerModal(false); setManagerToDelete(null); }}
                    title={`Sterge Manager: ${managerToDelete?.username}`}
                    footer={<>
                        <button className="btn btn-secondary" onClick={() => setShowDeleteManagerModal(false)}>Anuleaza</button>
                        <button className="btn btn-danger" onClick={handleDeleteManager}>Sterge Manager</button>
                    </>}>
                    {managerExecutors.length === 0 ? (
                        <p>Acest manager nu are executori asignati. Vrei sa il stergi?</p>
                    ) : (
                        <div>
                            <p style={{ marginBottom: '1rem' }}>
                                Acest manager are <strong>{managerExecutors.length} executori</strong> asignati:
                            </p>
                            ...
                            <div className="form-group">
                                <label className="form-label">Ce vrei sa faci cu executorii?</label>
                            ...
                            {deleteAction === 'reassign' && (
                                <div className="form-group" style={{ marginTop: '1rem' }}>
                                    <label className="form-label">Selecteaza noul manager:</label>
                                    <select
